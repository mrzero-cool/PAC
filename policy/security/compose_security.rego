...
# Description: Set 'read_only: true' to prevent modifications; use 'tmpfs' for writable dirs.
# Severity: High
warn contains msg if {
    input.services
    some service_name, service_config in input.services
    not service_config.read_only
    msg := sprintf("[DC_SEC_004][HIGH] Service '%s' should set 'read_only: true' to make the root filesystem immutable.", [service_name])
}

# DC_SEC_005: Use tmpfs for temporary files
# Description: Mount '/tmp' and '/run' as tmpfs with restricted flags when read_only is true.
# Severity: High
warn contains msg if {
    input.services
    some service_name, service_config in input.services
    ...
    msg := sprintf("[DC_SEC_005][HIGH] Service '%s' is read-only but lacks 'tmpfs' mounts for /tmp or /run.", [service_name])
}
